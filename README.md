...
| **Control loop**           | FC-HCC, closed end-to-end: sense → **certify** (worst-case admission, taper-aware) → schedule (feasibility-first, then price-aware) → compile → actuate with **CALLRESULT ack correlation** → **verify** against metered kW. Optimizer-**independent** envelope rejects cap-violating pushes (`-20903`), pushes are idempotent per (cp, decision) and rate-governed 6/min/CP; per-site mode defaults **OFF** until receipts exist — `apps/api/src/control/`, ADR-0010/0011/0014                                                                                                    |
| **Twin + experiments**     | Deterministic generative fleet (arrivals, SoC, CC-CV taper, ToU, non-compliant chargers) with a shared scoring harness — `apps/simulator/src/twin.js`. Receipts are reproducible from their seeds alone (scenarios are anchored to a fixed UTC epoch). **E1b**: controller −46.7 cost units vs static-cap (95% CI [−48.3, −45.0], 200 paired seeds), 0 calibration shortfall on 1,687 admitted vehicles. **E3**: admission tracks load; **and the taper-binding family fails** (42.6% shortfall, no deadline advantage over static-cap at ρ=0.9) — published as-is, `docs/perf.md` |
