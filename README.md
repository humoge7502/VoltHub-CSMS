...
> | 🔍 **Reviewer / sceptic**  | [ADRs](docs/adr/) (trade-offs named) → [`docs/verification.md`](docs/verification.md) (every claim has a receipt) → [`docs/perf.md`](docs/perf.md) (numbers, methodology) → [`SECURITY.md`](SECURITY.md)      |     | **66** | **2** | **7** | **13** | **6** | **0** |

| :-----------------------------------: | :------------------------------: | :----------------------------: | :--------------------: | :-------------------: | :---------------------------: |
| Spec'd API paths, OpenAPI drift-gated | DB engines behind one store port | PL/SQL packages own the writes | ADRs, trade-offs named | CI jobs on every push | known CVEs, `npm audit` gated |
...
flowchart TB
  ...
  subgraph API["apps/api — one port :4000"]
    REST["REST /api/v1 — 66 spec'd paths, OpenAPI drift-gated"]
...
| **OCPP 1.6J**              | WebSocket gateway with HTTP Basic auth on upgrade (Security Profile 1), per-CP credentials, 10 msg/s limit, 7 core messages + Smart Charging (`SetChargingProfile`/`Clear`/`GetCompositeSchedule`) + monotonic tick sequencing — `apps/api/src/ocpp/gateway.js`, `apps/api/src/ocpp/smart-charging.js`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Control loop (durable)** | The loop is proven through the DB engine, not just the local store: `test/e2e/control-plane.js` drives certify → plan → actuate → typed outcome → durable audit over HTTP against the compose stack and refuses to run unless `/health` reports `mode: oracle`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Control loop**           | FC-HCC, closed end-to-end: sense → **certify** (worst-case admission, taper-aware) → schedule (**acceptance-envelope**, feasibility-first then price-aware) → compile → actuate with **CALLRESULT ack correlation** → **verify** against metered kW. Optimizer-**independent** envelope rejects cap-violating pushes (`-20903`), pushes are idempotent per (cp, decision) and rate-governed 6/min/CP; per-site mode defaults **OFF** until receipts exist — `apps/api/src/control/`, ADR-0010/0011/0014/0015                                                                                                                                                                                                                                                                                                                                               |
| **Twin + experiments**     | Deterministic generative fleet (arrivals, SoC, CC-CV taper, ToU, non-compliant chargers) with a shared scoring harness — `apps/simulator/src/twin.js`. Receipts are reproducible from their seeds alone (scenarios are anchored to a fixed UTC epoch). **E1b**: controller −46.7 cost units vs static-cap (95% CI [−48.3, −45.0], 200 paired seeds), 0 calibration shortfall on 1,687 admitted vehicles. **E3**: admission tracks load; **and the taper-binding family fails** (42.6% shortfall, no deadline advantage over static-cap at ρ=0.9) — published as-is. **E6** repairs it without editing E3: acceptance-envelope scheduling cuts shortfall **42.6% → 2.36%** and deadline miss **56.6% → 15.7%**, with plan-honesty gap falling from **1,280 kWh → 0.011 kWh** per 100-seed run and **zero change below the knee** — `docs/perf.md`, ADR-0015 |
| **DA3 pipeline**           | Outbox → 2s relay → hypertables → caggs; crash-after-COPY replays safely — `apps/worker/`, `db/timescale/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **API**                    | 66 spec'd paths, OpenAPI 3.0 with a CI **drift gate** + a generated-snapshot check, Idempotency-Key replay, keyset pagination, request-ID error envelopes — `apps/api/src/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **CI**                     | 6 jobs: lint (Node 20/22 matrix) · security (npm audit gate, both lockfiles) · quality · coverage (c8 → Codecov) · db-tests (Oracle + Timescale service containers) · e2e (full compose). The control-loop properties, the AI advisory contract, the twin properties, the OpenAPI snapshot freshness and the **durable** control-plane journey (`npm run test:e2e:control`) all gate on every push — `.github/workflows/ci.yml`                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- OCPP scope is the **1.6J core profile + Smart Charging** (Boot/Heartbeat/Status/Authorize/Start/Meter/Stop, SetChargingProfile/ClearChargingProfile/GetCompositeSchedule); DataTransfer/diagnostics/firmware management are deliberately out of scope. Migration path to OCPP 2.0.1 (IEC 63584): `TransactionEvent` replaces Start/Meter/Stop — the store surface is protocol-neutral by design.
...
- Current release: **[v1.7.0](https://github.com/humoge7502/VoltHub-CSMS/releases)** — tagged releases carry notes extracted from [`CHANGELOG.md`](CHANGELOG.md) (Keep-a-Changelog / SemVer).
...
| Decision records                  | `docs/adr/0001` modular monolith · `0002` TimescaleDB (4.90/5) · `0003` outbox pipeline · `0004` plain-JS divergence · `0005` store adapter · `0006` connector FK-native · `0007` OCPP remote commands · `0008` AI advisory sidecar · `0009` multi-tenancy (deferred) · `0010` FC-HCC control loop · `0011` control-path integrity · `0012` twin profile compliance · `0013` benchmark methodology |
...
| REST contract (OpenAPI 3.0)       | [`docs/openapi.json`](docs/openapi.json) — **generated** snapshot of the live `/api/v1/docs` spec (`npm run openapi:gen`); route coverage drift-gated and snapshot freshness byte-checked in CI                                                                                                                                                                                                    |
