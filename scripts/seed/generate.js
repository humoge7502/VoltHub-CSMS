...
const violations = [];
const overlap = [...s.reservations.values()].filter(a => [...s.reservations.values()].some(b =>
  b.reservation_id > a.reservation_id && b.connector_ref === a.connector_ref &&
  ['BOOKED', 'CONVERTED'].includes(b.status) && ['BOOKED', 'CONVERTED'].includes(a.status) &&
  new Date(b.start_at) < new Date(a.end_at) && new Date(b.end_at) > new Date(a.start_at)));
...
for (const inv of s.invoices.values()) {
  if (['PAID', 'DUE'].includes(inv.status) && s.sessions.get(inv.session_id)?.state !== 'COMPLETED') violations.push(`invoice ${inv.invoice_id} w/o COMPLETED`);
  if (inv.status === 'PAID') {
...
for (const [sid, arr] of bySess) {
  ...
  for (let i = 1; i < arr.length; i++) if (arr[i].meter_kwh < arr[i - 1].meter_kwh - 0.001) violations.push(`meter regression session ${sid}`);
}
if (violations.length) { console.error('SEED INVARIANTS FAILED:', violations.slice(0, 10)); process.exit(1); }
console.log(`seed invariants: 0 violations (${s.reservations.size} res, ${s.sessions.size} sess, ${s.invoices.size} inv)`);
const out = path.join(__dirname, '..', '..', 'data', `seed-${profile}.json`);
fs.mkdirSync(path.dirname(out), { recursive: true });
const dump = {
  users: [...s.users.values()].map(u => ({ ...u, password_hash: 'REDACTED' })),
  stations: [...s.stations.values()], connectors: [...s.connectors.values()],
  plans: [...s.plans.values()], bands: s.bands,
  sessions: [...s.sessions.values()], invoices: [...s.invoices.values()],
};
