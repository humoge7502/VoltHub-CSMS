...
> Masterplan sections 16–17. The dataset must make queries _interesting_ (diurnal peaks, faults, abandoned reservations, price changes) and the query portfolio must read like evidence the schema was designed for real questions.
...
2. **Coherent:** meter ticks are consistent with session boundaries; invoices sum from tariff bands; reservation overlaps never exist; connector status history matches sessions. The generator calls the _same packages_ the API uses — seed data is valid because it was born through the valid paths.
...
_Purpose: the driver app's first query. Bounding box + status prefilter uses ix_station_geo before the view joins._
...
_Purpose: inventory report; exercises join + group by._
...
_Purpose: FR-DRV-03; LEFT JOIN because unbilled sessions still appear._
...
_Purpose: operator KPI; also demonstrates the encoded-key join trade-off (Part V, 10.3) honestly._
...
_Purpose: "full right now" badge; HAVING + CASE._
...
_Purpose: fault triage queue; correlated scalar subquery._
...
_Purpose: FR-MNT backlog; NOT EXISTS anti-join._
...
_Purpose: capacity planning; CTE + ratio-to-total window._
...
_Purpose: the KPI operators actually compete on (Section 3.7); RANK with ties._
...
_Purpose: smooths weekday noise for the trend chart; frame clause._
...
_Purpose: the sweep-line technique — how many chargers were busy at once; the seed of DA3's load curve done in pure SQL._
...
_Purpose: proves billing inputs; interval join._
...
_Purpose: stable pagination at depth vs OFFSET's O(n) scan; uses ix_session_user_time._
...
_Purpose: preventive-maintenance targeting._
...
_Purpose: demonstrates why versioned tariffs (Part VI, 12.7) enable analysis._
...
_Purpose: self-auditing data — should return zero rows; great "we test our own invariants" line._
...
_(Simulator emits IDLE_TIMEOUT stops; connects pricing rule FR-TAR idle_fee to data.)_
...
_Purpose: classic TOP-N-per-group with ROW_NUMBER — a guaranteed viva question._
...
_Ties to the arXiv Fault-Time KPI [9]._
...
_Purpose: fleet-planning signal (CCS2 share rising?)._
...
_Purpose: returns zero rows *always* — used as an automated DB test (Section 33) and demoed as the correctness proof._
...
_Purpose: one round-trip for the dashboard header cards._
