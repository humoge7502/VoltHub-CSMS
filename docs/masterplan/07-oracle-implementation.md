...
All tables live in the default tablespace; we do **not** pretend to tune tablespaces. What we _do_ address honestly: PCTFREE defaults are fine for our append-heavy tables; `TIMESTAMP(6)` everywhere (microseconds matter for meter ticks); `CLOB` only where JSON/JSON-check constraints are declared; numbers use explicit precision (`NUMBER(10,3)` for kWh) — a deliberate habit that looks professional and costs nothing.
