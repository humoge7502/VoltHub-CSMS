...
Tariffs exhibit temporal FDs: `plan(group_id) -> price` holds *at a time*, not absolutely. The naive design (mutable price column) creates update anomalies across history — changing a price would retroactively alter old invoices' meaning. The version-chain design (TARIFF_PLAN rows immutable, supersedes self-FK, sessions FK to the exact plan_id used) is **row versioning**: the "relation" is effectively `TARIFF(group_id, version_no, ...)` with a temporal key. This is 6NF-thinking (anchor + timeline) applied pragmatically without full 6NF machinery — worth one slide in the DA1 presentation.
