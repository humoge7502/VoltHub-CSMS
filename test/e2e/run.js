...
async function main() {
  ...
  let n = 0; const t = async (name, fn) => { await fn(); n++; console.log(`  e2e ${n} - ${name}`); };
  let tok, opTok;
  await t('driver registers with credit', async () => {
    const r = await api('/auth/register', { method: 'POST', body: JSON.stringify({ email: `e2e${Date.now()}@example.in`, password: 'Driver@123', full_name: 'E2E Driver' }) });
    assert.equal(r.status, 201); tok = r.j.accessToken;
  });
  ...
  await t('discovers a live CCS2 connector', async () => {
    ...
    assert.ok(s); stationId = s.station_id;
    conn = s.connectors.find(c => c.status === 'AVAILABLE');
  });
  let res;
  await t('reserves it', async () => {
    const [cp, no] = conn.connector_ref.split(':').map(Number);
    const r = await api('/reservations', { method: 'POST', headers: H(), body: JSON.stringify({ cpId: cp, connectorNo: no, startAt: new Date(Date.now() + 20 * 60000).toISOString(), endAt: new Date(Date.now() + 60 * 60000).toISOString() }) });
  ...
  await t('stops, bills, pays in full', async () => {
    ...
    const rev = await api(`/sessions/${sid}/review`, { method: 'POST', headers: H(), body: JSON.stringify({ rating: 5, comment: 'e2e verified electrons' }) });
    assert.equal(rev.status, 201);
  });
  await t('operator triages a fault', async () => {
    const op = await api('/auth/login', { method: 'POST', body: JSON.stringify({ email: 'arjun@volthub.in', password: 'Operator@123' }) });
    assert.ok(op.j.accessToken); opTok = op.j.accessToken;
    const OH = { Authorization: `Bearer ${opTok}` };
    const f = await api(`/stations/${stationId}/faults`, { method: 'POST', headers: OH, body: JSON.stringify({ connector_ref: conn.connector_ref, error_code: 'E2E-Test', severity: 'INFO', description: 'e2e fault' }) });
    assert.equal(f.status, 201);
    const m = await api(`/faults/${f.j.fault.fault_id}/maintenance`, { method: 'POST', headers: OH, body: JSON.stringify({ work_type: 'INSPECT', description: 'e2e check' }) });
    assert.equal(m.status, 201);
    const done = await api(`/maintenance/${m.j.record.record_id}/complete`, { method: 'PATCH', headers: OH, body: JSON.stringify({ resolution: 'e2e ok' }) });
