...
// Modes:
//  - local (default): boots an ephemeral API and evaluates the 7 invariants in
...
function localChecks(store) {
  ...
  // 1. No overlapping BOOKED/CONVERTED reservations.
  {
    ...
    for (let i = 0; i < rs.length; i++) for (let j = i + 1; j < rs.length; j++) {
      const a = rs[i], b = rs[j];
      if (a.connector_ref === b.connector_ref && new Date(a.start_at) < new Date(b.end_at) && new Date(a.end_at) > new Date(b.start_at)) {
  ...
  for (const [uid, w] of store.wallets) {
    ...
    if (Math.abs(w.balance - sum) > 0.005) fails.push(`INV-2 ledger drift user=${uid} balance=${w.balance} sum=${sum.toFixed(2)}`);
  }
  ...
  for (const inv of store.invoices.values()) {
    if (['PAID', 'DUE'].includes(inv.status)) {
      ...
      if (!s || s.state !== 'COMPLETED') fails.push(`INV-3 invoice ${inv.invoice_id} ${inv.status} on session state ${s?.state}`);
    }
  }
  ...
  for (const inv of store.invoices.values()) {
    if (['PAID', 'DUE'].includes(inv.status)) {
      ...
      if (Math.abs(inv.total - sum) > 0.005) fails.push(`INV-4 total drift invoice=${inv.invoice_id} total=${inv.total} lines=${sum.toFixed(2)}`);
    }
  }
  ...
  for (const sid of new Set(store.readings.map(r => r.session_id))) {
    ...
    for (let i = 1; i < rs.length; i++) {
      if (rs[i].meter_kwh < rs[i - 1].meter_kwh - 0.001) fails.push(`INV-5 regression session=${sid} seq=${rs[i].seq_no}`);
    }
  }
...
async function runOracleChecks() {
  ...
  try {
    ...
    for (const st of stmts) {
      ...
      const clean = st.split('\n').filter(l => !l.trim().startsWith('--')).join('\n').trim();
      if (!clean) continue;
      n++;
      const r = await conn.execute(clean);
      if ((r.rows || []).length) fails.push(`INV-SQL-${n}: ${r.rows.length} row(s): ${JSON.stringify(r.rows.slice(0, 3))}`);
      else console.log(`  inv-sql-${n}: 0 rows (pass)`);
    }
  } finally { try { await conn.close(); } catch {} try { await pool.close(); } catch {} }
  return fails;
}
...
async function main() {
  ...
  if (process.env.ORACLE_HOST) {
    try {
      ...
      if (fails.length) { console.error('INVARIANTS FAIL (oracle):\n' + fails.join('\n')); process.exit(1); }
  ...
  try {
    ...
    // Also assert the SQL file defines the same 7 checks (drift guard).
    const sql = fs.readFileSync(path.join(__dirname, '..', '..', 'db', 'oracle', 'invariants.sql'), 'utf8');
    const count = (sql.match(/\bSELECT\b/gi) || []).length;
    // File uses SELECT per check (some wrapped); require >= 7 SELECTs.
    if (count < 7) fails.push(`INV-META: invariants.sql defines ${count} SELECTs, expected >= 7`);
    if (fails.length) { console.error('INVARIANTS FAIL (local):\n' + fails.join('\n')); process.exitCode = 1; }
    else console.log(`invariants: local 7 checks × ${store.sessions.size} sessions / ${store.reservations.size} reservations — 0 rows — OK`);
  } finally { server.close(); }
  process.exit(process.exitCode || 0);
}
main().catch(e => { console.error('INVARIANTS ERROR', e); process.exit(1); });
