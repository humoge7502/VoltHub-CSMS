...
async function api(path, opts = {}) {
  ...
  if (!r.ok) { const e = new Error(j.error?.message || r.status); e.body = j; e.status = r.status; throw e; }
  return j;
}
let uid = 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    const h = (raw) => {
      try {
        ...
        if (m[1] === id) { clearTimeout(t); ws.off('message', h); m[0] === 4 ? reject(new Error('CALLERROR ' + m[2])) : resolve(m[2]); }
      } catch {}
    };
    ws.on('message', h);
  });
...
async function loginDriver() {
  ...
  const { accessToken } = await api('/auth/register', { method: 'POST', body: JSON.stringify({ email, password: 'Driver@123', full_name: 'Sim Driver' }) });
  return accessToken;
}
...
async function normalFlow(identity, cpId, connNo, token, faultMid = false) {
  ...
  const start = new Date(Date.now() + 20 * 60000), end = new Date(Date.now() + 60 * 60000);
  let reservation = null;
  try { ({ reservation } = await api('/reservations', { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify({ cpId, connectorNo: connNo, startAt: start.toISOString(), endAt: end.toISOString() }) })); } catch (e) { console.log(`[sim:${identity}] reserve skipped: ${e.message}`); }
  await ocppCall(ws, 'Authorize', { idTag: 'TAG-1' });
  const { transactionId } = await ocppCall(ws, 'StartTransaction', { connectorId: connNo, idTag: 'TAG-1', meterStart: 0, timestamp: new Date().toISOString() });
  console.log(`[sim:${identity}] charging tx=${transactionId} res=${reservation?.reservation_id ?? '-'}`);
  const ticks = faultMid ? 6 : 12;
  for (let i = 1; i <= ticks; i++) {
    if (faultMid && i === 5) {
      await ocppCall(ws, 'StatusNotification', { connectorId: connNo, status: 'Faulted', errorCode: 'GroundFailure', info: 'simulated ground fault' });
      console.log(`[sim:${identity}] FAULT injected mid-session`);
      break;
    }
    await ocppCall(ws, 'MeterValues', { connectorId: connNo, transactionId, meterValue: [{ timestamp: new Date().toISOString(), sampledValue: [{ value: String(i * 2500), measurand: 'Energy.Active.Import.Register', unit: 'Wh' }, { value: String(30000 + i * 500), measurand: 'Power.Active.Import', unit: 'W' }] }] });
    await new Promise(r => setTimeout(r, 700));
  }
  if (!faultMid) await ocppCall(ws, 'StopTransaction', { transactionId, meterStop: ticks * 2500, timestamp: new Date().toISOString() });
  ws.close();
...
async function main() {
  ...
  if (SCENARIO === 'race') {
    ...
    const c = cps[0]; const [cp, no] = c.connector_ref.split(':').map(Number);
    const s = new Date(Date.now() + 20 * 60000).toISOString(), e = new Date(Date.now() + 60 * 60000).toISOString();
    const mk = () => api('/reservations', { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Idempotency-Key': `race-${Math.random()}` }, body: JSON.stringify({ cpId: cp, connectorNo: no, startAt: s, endAt: e }) });
    const out = await Promise.allSettled([mk(), mk()]);
    console.log(out.map(x => x.status === 'fulfilled' ? '201 BOOKED' : `${x.reason.status || '?'} ${x.reason.message}`).join(' | '));
  ...
  if (SCENARIO === 'no-show') {
    const c = cps[0]; const [cp, no] = c.connector_ref.split(':').map(Number);
    const { reservation } = await api('/reservations', { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify({ cpId: cp, connectorNo: no, startAt: new Date(Date.now() + 20 * 60000).toISOString(), endAt: new Date(Date.now() + 50 * 60000).toISOString() }) });
  ...
  if (SCENARIO === 'burst') {
    ...
    await Promise.all(jobs); console.log('burst done'); return;
  }
  if (SCENARIO === 'fault-mid-session') {
    const c = cps[0]; const [cp, no] = c.connector_ref.split(':').map(Number);
    await normalFlow(c.ocpp_identity, cp, no, token, true); return;
  }
  for (let i = 0; i < N; i++) {
    const c = cps[i % cps.length]; const [cp, no] = c.connector_ref.split(':').map(Number);
    await normalFlow(c.ocpp_identity, cp, no, token);
  }
}
main().catch(e => { console.error('[sim] fatal', e.message, e.body || ''); process.exit(1); });
