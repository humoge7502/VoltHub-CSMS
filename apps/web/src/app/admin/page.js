...
export default function Admin() {
  ...
  const newVersion = async () => {
    const name = prompt('Version name', 'City Day v3');
    if (!name) return;
  ...
  return (
    <div className="wrap">
      ...
      <div style={{ display: 'flex', gap: 6, margin: '12px 0' }}>
        {['overview', 'stations', 'tariffs', 'audit'].map((t) => (
          <button
            key={t}
            className="btn"
            onClick={() => setTab(t)}
            style={tab === t ? { borderColor: 'var(--cream)' } : {}}
          >
            {t}
          </button>
        ))}
      ...
      {tab === 'overview' && (
        <div className="card">
          <div className="micro">CREATE OPERATOR (ADMIN ONLY)</div>
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            ...
            />
            <button className="btn pri" onClick={addUser}>
              Create
            </button>
          </div>
        </div>
      ...
      {tab === 'stations' && (
        <div className="grid">
          <div className="card">
            <div className="micro">PROVISION STATION (OCPP IDENTITIES AUTO-ASSIGNED)</div>
            <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
              ...
              <input
                ...
                style={{ maxWidth: 110 }}
              />
              <button className="btn pri" onClick={addStation}>
                Provision + 1×AC22
              </button>
            </div>
          </div>
            ...
            <tbody>
              {stations.map((s) => (
                <tr key={s.station_id}>
                  ...
                  <td>
                    <button className="btn" onClick={() => flipStation(s)}>
      ...
      {tab === 'tariffs' && (
        <div className="card">
          <div className="micro">VERSION TIMELINE · NEVER EDIT ACTIVE, ALWAYS SUPERSEDE</div>
          {plans.map((p) => (
            <div
              key={p.plan_id}
              style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: '1px solid var(--hair)' }}
            >
              <span className="num">
          ...
          <button className="btn pri" style={{ marginTop: 12 }} onClick={newVersion}>
            New version
          </button>
        </div>
      )}
