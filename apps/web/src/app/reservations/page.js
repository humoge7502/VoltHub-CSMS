    ...
    <div className="wrap">
          ...
          {list.map((r) => (
            <tr key={r.reservation_id}>
              ...
              <td style={{ display: 'flex', gap: 6 }}>
                {r.status === 'BOOKED' && (
                  <>
                    <button className="btn" onClick={() => cancel(r.reservation_id)}>
                      Cancel
                    </button>
                    <button className="btn pri" onClick={() => startSession(r)}>
                      Plug in
                    </button>
                  </>
                )}
      ...
      {!list.length && (
        <p style={{ color: 'var(--tx2)' }}>
          No bookings yet — <a href="/discover">find a charger</a>.
        </p>
      )}
    </div>
  );
