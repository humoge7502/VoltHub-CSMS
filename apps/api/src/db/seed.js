...
function seedStore(s, profile = 'demo') {
  ...
  const admin = {
    user_id: ++s.seq.user,
    email: 'admin@volthub.in',
    password_hash: hashPassword('Admin@123'),
    full_name: 'VoltHub Admin',
    role: 'ADMIN',
    status: 'ACTIVE',
  ...
  const ops = [];
  [
    ['arjun@volthub.in', 'Arjun Operator'],
    ['meera@volthub.in', 'Meera Operator'],
  ].forEach(([email, full_name]) => {
    const u = {
      user_id: ++s.seq.user,
      email,
      password_hash: hashPassword('Operator@123'),
      full_name,
      role: 'OPERATOR',
      status: 'ACTIVE',
      created_at: new Date().toISOString(),
    ...
    s.wallets.set(u.user_id, { user_id: u.user_id, balance: 0, currency: 'INR', updated_at: new Date().toISOString() });
    ops.push(u);
  });
  const drivers = [];
  const names = [
  ...
  for (let i = 0; i < (profile === 'full' ? 360 : 9); i++) {
    ...
    const u = {
      ...
      password_hash: hashPassword('Driver@123'),
      full_name: `${fn} ${ln}`,
      role: 'DRIVER',
      status: 'ACTIVE',
      created_at: new Date().toISOString(),
  ...
  const stationDefs = [
    [
      ...
      12.9716,
      80.0412,
      'Vandalur-Kelambakkam Rd, Chennai',
      'Chennai',
      'Tamil Nadu',
      '600127',
      ['CAFE', 'WIFI', 'PARKING', 'CCTV'],
    ],
    [
      'OMR Perungudi Hub',
      12.9698,
  ...
  stationDefs.forEach(([name, lat, lng, addr, city, state, pin, ams], i) => {
    ...
    s.stations.set(station_id, {
      ...
      pincode: pin,
      status: 'ACTIVE',
      operator_id: ops[i % ops.length].user_id,
      created_at: new Date().toISOString(),
    });
    ...
    for (let p = 0; p < 2; p++) {
      ...
      s.cps.set(cp_id, {
        ...
        station_id,
        ocpp_identity: ocpp,
        auth_secret: `dev-${ocpp}`,
        vendor: 'VoltHub',
        model: p === 0 ? 'VH-DC60' : 'VH-AC22',
      ...
      defs.forEach(([std, no, kw]) => {
        s.connectors.set(`${cp_id}:${no}`, {
          cp_id,
          connector_no: no,
          standard_id: std,
          max_power_kw: kw,
  ...
  if (s.connectors.get(`${faultCp}:2`)) {
    ...
    s.faults.set(fid, {
      fault_id: fid,
      connector_ref: `${faultCp}:2`,
      cp_id: faultCp,
      error_code: 'GroundFailure',
      severity: 'CRITICAL',
      source: 'OCPP',
      description: 'Ground fault detected mid-session (seed story)',
      reported_by: null,
      reported_at: new Date(Date.now() - 30 * 3600000).toISOString(),
      cleared_at: null,
    });
  }
    ...
    if (R() < 0.94) {
      try {
        ...
        s.invoices.set(inv, {
          ...
          total: amt,
          currency: 'INR',
          issued_at: end.toISOString(),
        });
        s.lines.push({
          invoice_id: inv,
          line_no: 1,
          kind: 'ENERGY',
          description: `Energy ${kwh} kWh @ Rs.${bandPrice}`,
          quantity: kwh,
          unit: 'kWh',
          unit_price: bandPrice,
          amount: +(kwh * bandPrice).toFixed(2),
        });
        if (fee > 0)
          s.lines.push({
            invoice_id: inv,
        ...
        if (w && w.balance >= amt && R() < 0.9) {
          ...
          s.ledgers.push({
            ...
            kind: 'PAYMENT',
            amount: -amt,
            balance_after: +(w.balance - amt).toFixed(2),
            payment_id: pid,
            note: `Invoice ${inv}`,
            created_at: end.toISOString(),
          });
