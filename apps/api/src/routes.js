...
module.exports = function routes(store) {
  ...
  r.post('/me/wallet/topup', authRequired, safe(async (req, res) => {
    res.json({ wallet: store.topup(req.user.id, Number(req.body.amount)) });
  }));
  // vehicles
  r.get('/me/vehicles', authRequired, safe(async (req, res) => {
  ...
  // ---- stations / discovery ----
  r.get('/stations', safe(async (req, res) => {
    const { q, std, minKw, lat, lng, radius, page = 1 } = req.query;
    let list = [...store.stations.values()].filter(s => s.status === 'ACTIVE');
    if (q) list = list.filter(s => (s.name + s.city + s.address_line).toLowerCase().includes(String(q).toLowerCase()));
    let rows = list.map(s => {
      const cps = [...store.cps.values()].filter(c => c.station_id === s.station_id);
      const conns = cps.flatMap(c => [...store.connectors.values()].filter(x => x.cp_id === c.cp_id).map(x => ({ ...x, ocpp_identity: c.ocpp_identity, connector_ref: `${x.cp_id}:${x.connector_no}`, standard_code: store.standards.find(t => t.standard_id === x.standard_id)?.code })));
      return { ...s, connectors: conns, available_count: conns.filter(c => c.status === 'AVAILABLE').length, connector_count: conns.length };
    });
    if (std) rows = rows.map(s => ({ ...s, connectors: s.connectors.filter(c => c.standard_code === std) })).filter(s => s.connectors.length);
    if (minKw) rows = rows.map(s => ({ ...s, connectors: s.connectors.filter(c => c.max_power_kw >= Number(minKw)) })).filter(s => s.connectors.length);
    if (lat && lng) { // haversine sort (km)
      const R = 6371, la = Number(lat) * Math.PI / 180, ln = Number(lng) * Math.PI / 180;
      rows.forEach(s => {
        const dLa = s.latitude * Math.PI / 180 - la, dLn = s.longitude * Math.PI / 180 - ln;
        const a = Math.sin(dLa / 2) ** 2 + Math.cos(la) * Math.cos(s.latitude * Math.PI / 180) * Math.sin(dLn / 2) ** 2;
    ...
    const per = 20, p = Math.max(1, Number(page));
    res.json({ stations: rows.slice((p - 1) * per, p * per), page: p, total: rows.length });
  }));
  r.get('/stations/:id', safe(async (req, res) => {
    const s = store.stations.get(Number(req.params.id));
    if (!s) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'station' } });
    const cps = [...store.cps.values()].filter(c => c.station_id === s.station_id).map(c => ({
      ...c, connectors: [...store.connectors.values()].filter(x => x.cp_id === c.cp_id).map(x => ({ ...x, connector_ref: `${x.cp_id}:${x.connector_no}`, standard_code: store.standards.find(t => t.standard_id === x.standard_id)?.code })),
    }));
    ...
    res.json({ station: { ...s, charge_points: cps, amenities: store.amenities.filter(a => a.station_id === s.station_id).map(a => a.amenity), avg_rating: stationRevs.length ? +(stationRevs.reduce((a, x) => a + x.rating, 0) / stationRevs.length).toFixed(2) : null, review_count: stationRevs.length } });
  }));
  r.get('/stations/:id/connectors/live', safe(async (req, res) => {
    const sid = Number(req.params.id);
    const rows = [...store.connectors.values()].filter(c => store.cps.get(c.cp_id)?.station_id === sid)
      .map(c => ({ connector_ref: `${c.cp_id}:${c.connector_no}`, ...c, standard_code: store.standards.find(t => t.standard_id === c.standard_id)?.code, ocpp_identity: store.cps.get(c.cp_id)?.ocpp_identity }));
    res.json({ connectors: rows });
  }));

  // ---- reservations ----
  r.post('/reservations', authRequired, safe(async (req, res) => {
    // Idempotency-Key replay (R5)
    const ik = req.headers['idempotency-key'];
    if (ik) {
      const hit = store.idem.get(`${req.user.id}:${ik}`);
      if (hit) return res.status(hit.status_code).json(JSON.parse(hit.response_body));
    }
    vReservation(req.body);
    try {
      const reservation = await store.createReservation(req.user.id, req.body.vehicleId, req.body.cpId, req.body.connectorNo, req.body.startAt, req.body.endAt);
      const body = JSON.stringify({ reservation });
      if (ik) store.idem.set(`${req.user.id}:${ik}`, { status_code: 201, response_body: body });
      res.status(201).json({ reservation });
    } catch (e) { res.status(oraStatus(e)).json({ error: { code: e.code || 'RESERVE_FAILED', message: e.message, ora: e.num || null } }); }
  }));
  r.get('/reservations', authRequired, safe(async (req, res) => {
  ...
  r.post('/reservations/:id/cancel', authRequired, safe(async (req, res) => {
    try { res.json({ reservation: await store.cancelReservation(req.params.id, req.user.id, req.user.role) }); }
  ...
  r.get('/sessions', authRequired, safe(async (req, res) => {
    ...
    if (req.user.role === 'DRIVER' || req.query.userId) {
      ...
      if (req.user.role === 'DRIVER' && uid !== req.user.id) return res.status(403).json({ error: { code: 'FORBIDDEN', message: '' } });
    ...
    res.json({ sessions: page, nextCursor: last ? Buffer.from(`${last.started_at}|${last.session_id}`).toString('base64') : null });
  }));
  r.post('/sessions/:id/bill', authRequired, safe(async (req, res) => {
    try { res.status(201).json({ invoice: await store.billSession(req.params.id) }); }
    catch (e) { res.status(oraStatus(e)).json({ error: { code: e.code, message: e.message } }); }
  }));

  // ---- billing ----
  r.get('/invoices', authRequired, safe(async (req, res) => {
    let list = [...store.invoices.values()];
    if (req.user.role === 'DRIVER') {
      const mine = new Set([...store.sessions.values()].filter(s => s.user_id === req.user.id).map(s => s.session_id));
  ...
  r.get('/invoices/:id', authRequired, safe(async (req, res) => {
    const i = store.invoices.get(Number(req.params.id));
    if (!i) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'invoice' } });
    res.json({ invoice: i, lines: store.lines.filter(l => l.invoice_id === i.invoice_id), payments: [...store.payments.values()].filter(p => p.invoice_id === i.invoice_id) });
  }));
  r.post('/invoices/:id/pay', authRequired, safe(async (req, res) => {
    try { res.status(201).json({ payment: await store.payInvoice(req.params.id, req.user.id) }); }
    catch (e) { res.status(oraStatus(e)).json({ error: { code: e.code || 'PAY_FAILED', message: e.message } }); }
  }));

  // ---- faults / maintenance ----
  r.post('/stations/:id/faults', authRequired, safe(async (req, res) => {
    const fid = ++store.seq.fault;
    const f = { fault_id: fid, connector_ref: req.body.connector_ref || null, cp_id: req.body.cpId || null, error_code: req.body.error_code || 'ManualReport', severity: req.body.severity || 'WARN', source: 'MANUAL', description: req.body.description || null, reported_by: req.user.id, reported_at: new Date().toISOString(), cleared_at: null };
    store.faults.set(fid, f);
    if (f.connector_ref && store.connectors.get(f.connector_ref)) store.connectors.get(f.connector_ref).status = 'FAULTED';
    store.auditLog(req.user.id, 'FAULT', fid, 'REPORT', null, f.error_code);
    res.status(201).json({ fault: f });
  }));
  r.get('/faults', authRequired, safe(async (req, res) => {
  ...
  r.post('/faults/:id/maintenance', authRequired, roles('OPERATOR', 'ADMIN'), safe(async (req, res) => {
    const rid = ++store.seq.maint;
    const m = { record_id: rid, fault_id: Number(req.params.id), performed_by: req.user.id, work_type: req.body.work_type || 'INSPECT', description: req.body.description || '', cost: Number(req.body.cost || 0), started_at: new Date().toISOString(), resolved_at: null, resolution: null };
    store.maint.set(rid, m);
    res.status(201).json({ record: m });
  }));
  r.patch('/maintenance/:id/complete', authRequired, roles('OPERATOR', 'ADMIN'), safe(async (req, res) => {
    const m = store.maint.get(Number(req.params.id));
    if (!m) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'record' } });
    m.resolved_at = new Date().toISOString(); m.resolution = req.body.resolution || 'resolved';
    const f = store.faults.get(m.fault_id);
    if (f) { f.cleared_at = new Date().toISOString(); if (f.connector_ref && store.connectors.get(f.connector_ref)?.status === 'FAULTED') store.connectors.get(f.connector_ref).status = 'AVAILABLE'; }
    store.auditLog(req.user.id, 'MAINTENANCE_RECORD', m.record_id, 'RESOLVE', null, m.resolution);
    res.json({ record: m });
  }));

  // ---- analytics (operator) ----
  r.get('/stations/:id/analytics', authRequired, safe(async (req, res) => {
    const sid = Number(req.params.id);
    const keys = new Set([...store.cps.values()].filter(c => c.station_id === sid).map(c => String(c.cp_id)));
    const sess = [...store.sessions.values()].filter(s => keys.has(s.connector_ref.split(':')[0]) && s.state === 'COMPLETED');
    const invs = sess.map(s => [...store.invoices.values()].find(i => i.session_id === s.session_id)).filter(Boolean);
    res.json({
      revenue: +invs.filter(i => ['PAID', 'DUE'].includes(i.status)).reduce((a, i) => a + i.total, 0).toFixed(2),
      energy_kwh: +sess.reduce((a, s) => a + (s.energy_kwh || 0), 0).toFixed(3),
      sessions: sess.length,
      active: [...store.sessions.values()].filter(s => keys.has(s.connector_ref.split(':')[0]) && ['PREPARING', 'CHARGING', 'SUSPENDED'].includes(s.state)).length,
      open_faults: [...store.faults.values()].filter(f => !f.cleared_at && f.connector_ref && keys.has(f.connector_ref.split(':')[0])).length,
    });
  }));
  ...
  r.get('/telemetry/load-curve', authRequired, safe(async (req, res) => {
    ...
    if (station) {
      const keys = new Set([...store.cps.values()].filter(c => c.station_id === Number(station)).map(c => String(c.cp_id)));
    ...
    ticks.forEach(t => {
      ...
      const o = buckets.get(b); o.sum += t.power_kw || 0; o.max = Math.max(o.max, t.power_kw || 0); o.n++;
    });
  ...
  r.get('/telemetry/utilization-heatmap', authRequired, safe(async (req, res) => {
    ...
    store.stateEvents.forEach(e => {
      const d = new Date(e.ts); grid[d.getDay()][d.getHours()] += e.to_state === 'OCCUPIED' ? 1 : 0;
    });
    res.json({ heatmap: grid });
  }));

  // ---- reviews ----
  r.post('/sessions/:id/review', authRequired, safe(async (req, res) => {
    const sid = Number(req.params.id);
    if ([...store.reviews.values()].some(x => x.session_id === sid)) return res.status(409).json({ error: { code: 'DUPLICATE_REVIEW', message: 'one review per session' } });
    const rid = ++store.seq.review;
    const rev = { review_id: rid, session_id: sid, user_id: req.user.id, rating: Number(req.body.rating), comment_text: req.body.comment || null, created_at: new Date().toISOString() };
    if (!(rev.rating >= 1 && rev.rating <= 5)) return res.status(422).json({ error: { code: 'INVALID_RATING', message: '1..5' } });
    store.reviews.set(rid, rev);
    res.status(201).json({ review: rev });
  }));

  // ---- admin ----
  r.get('/admin/audit-logs', authRequired, roles('ADMIN'), safe(async (req, res) => {
    res.json({ logs: store.audit.slice(-200).reverse() });
  }));
  r.post('/admin/users', authRequired, roles('ADMIN'), safe(async (req, res) => {
    const u = store.createUser({ email: req.body.email, password: req.body.password || 'Temp@1234', full_name: req.body.full_name, role: req.body.role || 'OPERATOR' });
    if (req.body.stationId) { const s = store.stations.get(Number(req.body.stationId)); if (s) s.operator_id = u.user_id; }
    res.status(201).json({ user: pub(u) });
  }));
  r.patch('/admin/users/:id', authRequired, roles('ADMIN'), safe(async (req, res) => {
  ...
  r.post('/admin/tariff-plans', authRequired, roles('ADMIN'), scopeCheck, safe(async (req, res) => {
  ...
  r.get('/admin/tariff-plans', authRequired, roles('ADMIN', 'OPERATOR'), safe(async (req, res) => {
