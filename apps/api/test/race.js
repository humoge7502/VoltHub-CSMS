...
async function main() {
  ...
  const { j: reg } = await api('/auth/register', { method: 'POST', body: JSON.stringify({ email: `race${Date.now()}@example.in`, password: 'Driver@123', full_name: 'Race Driver' }) });
  ...
  const st = await api('/sessions/start', { method: 'POST', headers: H, body: JSON.stringify({ cpId: cp2, connectorNo: no2, planId: 2 }) });
...
main().catch(e => { console.error('RACE FAIL', e.message); process.exit(1); });
