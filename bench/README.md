...
| 8   | `e3-deadline.js`           | **promise-keeping under overload**: admission refusal, certificate calibration and deadline-miss vs load (ρ 0.6/0.9/1.2) in two SoC families (target 80%, target 95% where the CC-CV taper binds). Committed as the **pre-fix record**; the taper-binding family's 42.6% shortfall is the defect E6 repairs |
| 9   | `e6-taper.js`              | **acceptance-envelope scheduling (AES, ADR-0015)**: the same ladder as E3 plus a paired `controller_aes` arm, testing H0 no-regression below the knee, H1 shortfall ≤ 5%, H2 deadline advantage, H3 cost/kWh, H4 plan honesty (`plan ≈ delivered`). Repairs 42.6% → 2.36% without editing E3                |

E1b/E3/E6 run in seconds and need no Docker or k6: `npm run bench:e1b`, `npm run bench:e3`,
`npm run bench:e6` (receipts: `bench/results/e1b-cost.json`, `bench/results/e3-deadline.json`,
`bench/results/e6-taper.json`). Their workload
...
5. **A failed hypothesis exits 0 and is published.** Only an envelope (planned-peak)
   violation exits non-zero: safety is not a statistic. A **repair** is a new id that
   _supersedes the diagnosis_, never an edit to the failing receipt — E3 stays committed
   as the pre-fix record and E6 carries the fix (`bench/e6-taper.js`).
